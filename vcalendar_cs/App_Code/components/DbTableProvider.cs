...
{
public enum Condition{Equal,NotEqual,LessThan,LessThanOrEqual,GreaterThan,GreaterThanOrEqual,BeginsWith,NotBeginsWith,EndsWith,NotEndsWith,Contains,NotContains,NotNull,IsNull,DefaultNull, Between, NotBetween, In, NotIn}
public class TableCommand:DataCommand
{
    ...
    public void AddParameter(string paramName,Parameter param, string format, string field, Condition op, bool useDefaultNull)
    {
        ...
        if(useDefaultNull && param == null)
            op = Condition.IsNull;
        else if(param != null && op != Condition.Between && op != Condition.NotBetween)
            val = param.GetFormattedValue(format);
        else if((op == Condition.Between || op == Condition.NotBetween) && !param.IsNull){
    ...
    protected void AddParameter(string paramName,string param, string param1, FieldType fieldType, string field, Condition op)
    {
        ...
        case Condition.Between:
                Parameters.Add(paramName,field + " BETWEEN " +  Dao.ToSql(param, fieldType, true) + " AND " +  Dao.ToSql(param1, fieldType, true));return;
        case Condition.NotBetween:
                Parameters.Add(paramName,field + " NOT BETWEEN " +  Dao.ToSql(param, fieldType, true) + " AND " +  Dao.ToSql(param1, fieldType, true));return;
        case Condition.In:
                Parameters.Add(paramName,field + " IN(" + param + ")" );return;
        case Condition.NotIn:
                Parameters.Add(paramName,field + " NOT IN(" + param + ")" );return;
        }
    }
